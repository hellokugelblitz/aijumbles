...
function App() {
  ...
  const generateVideo = () => {
    return new Promise((resolve, reject) => {
      if (storyArray) {
        ...
        fetch('http://3.17.145.87:5000/generate-video', {
          method: 'POST',
          headers: {
  ...
  const startStory = (newStoryType, storyTitle) => {
    ...
    fetch(`http://3.17.145.87:5000/generate-script?input_string=${newStoryType}`)
        .then(response => {
  ...
  const cleanupBackend = async () => {
    try {
      const response = await fetch('http://3.17.145.87:5000/cleanup-usercode', {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json'
        },
          ...
          <h1 className="story-title">Nature Documentary</h1>
            <div className="word-menu">
              <div className="word-list">
                  <input className="word-input" autoComplete="off" id="word-input" type="text" name="name" maxlength="20" tabindex="-1" />
                  <p>Submit any Word</p>
              </div>
            </div>
